...
void glDrawElementsBaseVertex(GLenum mode, GLsizei count, GLenum type, const void* indices, GLint basevertex) {
    ...
    if (hardware->es_version < 320 && !g_gles_caps.GL_EXT_draw_elements_base_vertex &&
        ...
        if ((GLsizeiptr)(count * indexSize) > s_temp_buffer_size) {
            GLES.glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, s_temp_buffer);
            GLES.glBufferData(GL_ELEMENT_ARRAY_BUFFER, count * indexSize, nullptr, GL_DYNAMIC_DRAW);
            s_temp_buffer_size = count * indexSize;
        }
        ...
        if (prevElementBuffer != 0) {
            ...
            if (srcData) {
                memcpy(mapped, srcData, count * indexSize);
            ...
            memcpy(mapped, indices, count * indexSize);
        }
